...
class SocketConn:
    ...
    def _connect(self, retries:int):
        while True:
            try:
                ...
                if retries == 0:
                    print(f"Couldn't reach host at {self.address}", flush=True)
                    raise err
                retries_str = " " if retries < 0 else str(retries)+ " more time(s) "
                print(f"Couldn't reach host at {self.address}, retrying {retries_str}in {self.retry_interval} seconds...", flush=True)
    ...
    def close(self):
        try:
            ...
            print(f"Could not close connection due to '{err}'", flush=True)
    ...
    def consumer(self):
        ...
        while True:
            try:
                ...
                print(f"Could not consume message due to '{err}'", flush=True)
                if retries > 0:
                    retries -= 1
                    ...
                    print(f"Failed to consume message", flush=True)
                    raise err
        self.close()
    ...
    def produce(self, data):
        ...
        while True:
            try:
                ...
                print(f"Could not send data due to '{err}'", flush=True)
                if retries > 0:
                    retries -= 1
                    print(f"Retrying connection...", flush=True)
                    self._silent_reconnect(0)
